...
SD_CARD_IMG="$(pwd)/imx-yocto-bsp/build-xwayland-aiml/tmp/deploy/images/imx8qxpaiml/imx-image-full-imx8qxpaiml.wic.bz2"
red=`tput setaf 1`
green=`tput setaf 2`
bold=`tput bold`
reset=`tput sgr0`
...
meta_imx_head=780b023a4c77ac9c0011d858ad377f4a9593bfea
meta_qt5_head=5144a99a696a8a72c0765ea0e8be8ceafaa78f85
meta_openembedded_head=2b5dd1eb81cd08bc065bc76125f2856e9383e98b
meta_browser_head=ee3be3b5986a4aa0e73df2204a625ae1fe5df37e
poky_head=5408e48d374fc741fa46b57ddfd8a537843bda6f
...
setup_ml_data_models()
{
	...
	then
		...
		echo "${red}https://github.com/ArrowElectronics/meta-einfochips/releases/download/v3.1-imx/ML-data-models.zip please check here${reset}"
		echo "###################################################################################"
		exit 1
	fi
...
apply_patch()
{
	...
	# copy meta-quectel-community layer and meta-laird-cp layer
	if [ ! -d meta-quectel-community ]
	then
		cp -r $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-quectel-community/ .
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
			echo "${red}Error during apply the patch in meta-quectel-community"
			echo "Please verify AIML_L5_4_47_Rel_3_1_patches directory${reset}"
			echo "###################################################################################"
			exit 1
		fi
	fi
	...
	if [ ! -d meta-laird-cp ]
	then
		cp -r $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-laird-cp/ .
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
			echo "${red}Error during apply the patch in meta-laird-cp"
			echo "Please verify AIML_L5_4_47_Rel_3_1_patches directory${reset}"
			echo "###################################################################################"
			exit 1
		fi
	...
	git apply --check -R $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-imx-patch/00* 2>/dev/null
	if [ $? -ne 0 ]
	then
		echo "###################################################################################"
		...
		git am --whitespace=fix $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-imx-patch/00*
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
	...
	git apply --check -R $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-qt5-patch/00* 2>/dev/null
	if [ $? -ne 0 ]
	then
		echo "###################################################################################"
		...
		git am $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-qt5-patch/00*
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
	...
	cd $SOURCE_DIR/poky
        git apply --check -R $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/poky-patch/00* 2>/dev/null
        if [ $? -ne 0 ]
        then
                echo "###################################################################################"
                echo "Apply the patch in poky"
                echo "###################################################################################"
                git checkout -f $poky_head
                git am $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/poky-patch/00*
                if [ $? -ne 0 ]
                then
                        echo "###################################################################################"
                        echo "${red}Error during apply the patch in poky"
                        echo "Please verify AIML_L5_4_47_Rel_3_1_patches directory${reset}"
                        echo "###################################################################################"
                        git format-patch $poky_head
                        exit 1
                fi
        fi

	cd $SOURCE_DIR/meta-openembedded
        git apply --check -R $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-openembedded-patch/00* 2>/dev/null
        if [ $? -ne 0 ]
        then
                echo "###################################################################################"
                ...
                git am $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-openembedded-patch/00*
                if [ $? -ne 0 ]
                then
                        echo "###################################################################################"
	...
	cd $SOURCE_DIR/meta-browser
        git apply --check -R $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-browser-patch/00* 2>/dev/null
        if [ $? -ne 0 ]
        then
                echo "###################################################################################"
                ...
                git am $YOCTO_DIR/AIML_L5_4_47_Rel_3_1_patches/meta-browser-patch/00*
                if [ $? -ne 0 ]
                then
                        echo "###################################################################################"
...
build_image()
{
	...
	if [ $? -ne 0 ];then
		...
		echo "You can find imx-image-full-imx8qxpaiml.wic.bz2 image at below location"
		echo "$(pwd)/tmp/deploy/images/imx8qxpaiml/"
		echo "###################################################################################${reset}"
	fi
}
...
then
	echo "${green}###################################################################################"
	echo "SD card image imx-image-full-imx8qxpaiml.wic.bz2 exists. Your yocto setup is already up to date"
	echo "###################################################################################${reset}${reset}"
else
	# Check prerequisite
	prerequisite
